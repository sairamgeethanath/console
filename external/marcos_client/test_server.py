...
class ServerTest(unittest.TestCase):
    ...
    def test_idle(self):
        ...
        def check_status():
            ...
            if (status_latch_reg & fhdo_err) or (status_latch_reg & ocra1_err) or (status_latch_reg & ocra1_data_lost):
                warnings.warn(
                    "Gradient error occurred during test_idle! Might have been caused by another of the tests.")
    ...
    def test_bus(self):
        ...
        if print_speeds:
            print(
                f"{real} data: null_t: {null_t / loops:.2f}, read_t: {read_t / loops:.2f}, write_t: {write_t / loops:.2f} us / cycle")
        if real == "hardware":
            self.assertAlmostEqual(null_t / 1e3, times[0] * loops_norm,
                                   delta=deltas[0] * loops_norm)  # 1 flop takes ~1.5 ns on average
            self.assertAlmostEqual(read_t / 1e3, times[1] * loops_norm,
                                   delta=deltas[1] * loops_norm)  # 1 read takes ~141.9 ns on average
            self.assertAlmostEqual(write_t / 1e3, times[2] * loops_norm,
                                   delta=deltas[2] * loops_norm)  # 1 write takes ~157.9 ns on average
        elif real == "simulation":
    ...
    def test_several_some_bad(self):
        ...
        self.assertEqual(reply,
                         [reply_pkt, 1, 0, version_full,
                          {'lo_freq': 0, 'tx_div': -1, 'rx_div': -1, 'tx_size': -1, 'raw_tx_data': -1, 'grad_div': -1,
                           'grad_ser': -1, 'grad_mem': -1, 'acq_rlim': -1},
    ...
    def test_grad_adc(self):
        ...
        send_packet(construct_packet({'direct': 0x00000000 | (2 << 0) | (spi_div << 2) | (0 << 8) | (upd << 9)}),
                    self.s)
    ...
    def test_mar_mem(self):
        ...
        self.assertEqual(reply,
                         [reply_pkt, 1, 0, version_full,
                          {'mar_mem': -1},
                          {'errors': [
                              'too much mar mem data: {:d} bytes > {:d} -- streaming not yet implemented'.format(
                                  mar_mem_bytes + 1, mar_mem_bytes)]}
                          ])
